      ...
      <v-col cols="8">
        ...
        <v-dialog v-model="cadastroModal" max-width="600">
          <v-card>
            ...
            <v-card-text>
              <!-- Campos de entrada -->
              <v-text-field v-model="newStudent.name" label="Nome"></v-text-field>
              <v-text-field v-model="newStudent.email" label="E-mail"></v-text-field>
              <v-text-field v-model="newStudent.ra" label="RA"></v-text-field>
              <v-text-field v-model="newStudent.cpf" label="CPF"></v-text-field>
            </v-card-text>
            <v-card-actions>
              <!-- Botões de ação -->
        ...
        <v-dialog v-model="editModal" max-width="600">
          <v-card>
            ...
            <v-card-text>
              <!-- Campos de entrada -->
              <v-text-field v-model="selectStudent.name" label="Nome"></v-text-field>
              <v-text-field v-model="selectStudent.email" label="E-mail"></v-text-field>

              <!-- Campos não editáveis -->
              <v-text-field v-model="selectStudent.ra" label="RA" readonly></v-text-field>
              <v-text-field v-model="selectStudent.cpf" label="CPF" readonly></v-text-field>
            </v-card-text>
            <v-card-actions>
              <!-- Botões de ação -->
...
const saveStudent = async () => {
  try {
    const studentJson = JSON.stringify(newStudent.value);

    const response = await axios.post('http://localhost:3000/student', studentJson, {
      headers: {
...
const saveEdit = async () => {
  if (selectStudent.value) {
    try {
      const alunoJson = JSON.stringify(selectStudent.value);
      await axios.patch(`http://localhost:3000/student/${selectStudent.value.ra}`, alunoJson, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
